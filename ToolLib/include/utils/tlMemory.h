...
#define DYNAMIC_SIZING 2 // Every time a dynamic memory allocator runs out of room, up its allocation by this multiplier 
#define NUM_SIZE_CLASSES 16 // Allow up to 2^16 bytes. 1GB Max memory allocated at once
...
{
  struct tlMemBucket *pBucket; // Pointer to the next bucket. the next free bucket basically
} tlMemBucket;
...
///@param searchStreams if interested in searching existing streams for space. mainly for static 
...
/// for Dynamic Allocations. Packs array room + a generous buffer for each reallocation
/// Best to reserve bytes in advance rather than call this repetitively
/// @param size memory in bytes to allocate
/// @return a ptr to the allocated memory
