...
void *CreateNewBucket(const int64_t size, tlMemoryManager *pManager) {
  ...
  if (pManager->m_pFreeHeads[bucketIndex] == NULL) {
    ...
    uint8_t *newAddress =
        (uint8_t *)(pManager->pArena + pManager->m_arenaPosition);
...
void *tlNewDynamicAlloc(const int64_t size) { return tlDynamicAlloc(size, 0); }
...
tlBool tlMemCompare(const void *pValueA, const void *pValueB,
                    const int64_t comparisonSize) {
  tlBool ret = FALSE;
  for (int i = 0; i < comparisonSize; ++i)
    if (*(uint8_t *)(pValueA + i) == *(uint8_t *)(pValueB + i))
      ret = TRUE;

  return ret;
}

void FreeBucket(void *pData, const int64_t bucketIndex,
                tlMemoryManager *pManager) {
...
tlBool tlFree(void *pData, const int64_t size, const AllocationType type) {
  ...
  if (!(type == DYNAMIC || type == STATIC))
    return FALSE;
  FreeBucket(pData, bucketIndex,
             (type == DYNAMIC) ? &dynamicManager : &staticManager);
  pData = NULL;
  return TRUE;
}
