...
tlVector tlVecCreate(const void *pData, const int64_t length,
...
tlVector tlVecCopy(const tlVector *pVector) {
  ...
  tlMemCopy(pVector->pData, pVector->elementSize * pVector->size,
            newVector.pData);
...
void tlVecSwapIndex(tlVector *pVector, const int64_t indexA,
                    const int64_t indexB) {
...
tlVector tlVecSwap(tlVector **pVector) {
  ...
  newVector.elementSize = (*pVector)->elementSize;
  newVector.size = (*pVector)->size;
  tlFree((*pVector)->pData, (*pVector)->size * (*pVector)->elementSize,
         DYNAMIC);
  return newVector;
}
...
void tlVecRemove(tlVector *pVector, const int64_t index) {
  for (int64_t element = index + 1; element < pVector->size; ++element) {
    ...
    *((uint8_t *)pVector->pData + prevPosition) =
        *((uint8_t *)pVector->pData + bytePosition);
  }
}
...
int64_t tlVecGetIndexOf(const tlVector *pVector, const void *pElement) {
  for (int64_t index = 0; index < pVector->size; ++index)
    if (*(int8_t *)pElement ==
        *(uint8_t *)pVector->pData + (index * pVector->elementSize))
      return index;
...
tlVector tlVecSort(const tlVector *pVector,
                   tlBool (*lambdaFunction)(const void *a, const void *b)) {
  int64_t index = 0;
  tlVector outputVector = *pVector;
  tlBool swapped = FALSE;
  for (int64_t i = 0; i < pVector->size; ++i) {
    for (int64_t j = 0; j < pVector->size - 1; ++j) {
      if (!lambdaFunction(tlVecAt(&outputVector, j),
